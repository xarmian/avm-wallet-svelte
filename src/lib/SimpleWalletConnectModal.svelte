<script lang="ts">
	...
	$: displayUri =
		walletName === 'VoiWalletx' ? `https://www.getvoi.app/wc?uri=${encodeURIComponent(uri)}` : uri;
...
{#if show}
	...
	<div
		...
		class="fixed inset-0 bg-black/60 dark:bg-black/70 backdrop-blur-md flex items-center justify-center p-4 transition-opacity duration-300 z-50"
	>
		<!-- Modal container -->
		<div
			class="bg-white dark:bg-gray-900 rounded-2xl max-w-md w-full shadow-2xl transform transition-all duration-300 scale-100 border border-gray-200 dark:border-gray-700"
		>
			<!-- Header -->
			<div class="relative p-6 pb-4 border-b border-gray-100 dark:border-gray-800">
				...
				<p class="text-sm text-center text-gray-500 dark:text-gray-400 mt-2">
					Scan the QR code or open on your mobile device
				</p>
				<button
					on:click={closeModal}
			...
			<div class="p-8 space-y-6">
				...
				<div class="flex flex-col items-center">
					{#if qrCodeDataUrl && qrCodeDataUrlDark}
						<div
							class="relative p-6 bg-gradient-to-br from-gray-50 to-gray-100 dark:from-gray-800 dark:to-gray-900 rounded-2xl shadow-inner"
						>
							<!-- Light theme QR code -->
							<img
								src={qrCodeDataUrl}
							...
							<div
								class="absolute top-3 left-3 w-6 h-6 border-t-3 border-l-3 border-blue-500 rounded-tl-lg"
							></div>
							<div
								class="absolute top-3 right-3 w-6 h-6 border-t-3 border-r-3 border-blue-500 rounded-tr-lg"
							></div>
							<div
								class="absolute bottom-3 left-3 w-6 h-6 border-b-3 border-l-3 border-blue-500 rounded-bl-lg"
							></div>
							<div
								class="absolute bottom-3 right-3 w-6 h-6 border-b-3 border-r-3 border-blue-500 rounded-br-lg"
							></div>
						</div>
					{:else}
						<div
							class="w-72 h-72 bg-gradient-to-br from-gray-50 to-gray-100 dark:from-gray-800 dark:to-gray-900 rounded-2xl flex items-center justify-center shadow-inner"
						>
