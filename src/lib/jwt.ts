...
export async function verifySignedTransaction(
	...
	if (payload.sub !== expectedAddress || payload.exp < Math.floor(Date.now() / 1000)) {
		...
		try {
			...
			try {
				...
				const publicKeysMatch =
					txn.payment?.receiver &&
					txn.sender.publicKey.length === txn.payment.receiver.publicKey.length &&
					txn.sender.publicKey.every(
						(value, index) => value === txn.payment!.receiver.publicKey[index]
					);
