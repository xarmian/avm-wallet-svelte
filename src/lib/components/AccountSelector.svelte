<script lang="ts">
	...
	let {
		...
		autoSignIn = false, // eslint-disable-line @typescript-eslint/no-unused-vars
		onAddAccount,
		onClose
	}: Props = $props();
	...
	const accountsByWallet = $derived(() => {
		...
		const grouped = new Map<
			string,
			{ walletId: string; walletName: string; icon?: string; accounts: typeof accounts }
		>();
				...
				<div class="account-selector__wallet-group">
					<div class="account-selector__wallet-header">
						...
						<button
							...
							onclick={(e) => {
								e.stopPropagation();
								handleDisconnect(wallet.accounts[0]);
							}}
