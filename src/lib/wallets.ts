...
export const wallets: Wallet[] = [
	...
	},
	{
		...
		connect: async () => {
			...
			if (!genesis) {
				throw new Error(
					'Genesis ID not found: algodClient handle needed to connect to Lute Wallet'
				);
			}
...
export const verifyToken = async (wallet: string, token: string): Promise<boolean> => {
	...
	if (
		...
		Number(decodedNote[1]) > Date.now() + day90 + minutes30
	) {
...
export async function verifySignature(signedTxn: Uint8Array): Promise<boolean> {
	...
	const isValidSignature = ed.verify(
		Buffer.from(decodedTxn.sig ?? new Uint8Array()).toString('base64'),
		txnBytes,
...
export async function verifyTransactionGroupSignatures(
	...
	if (signedTxns.length !== txns.length) {
		throw new Error(
			'The lengths of signed transactions, transactions, and public keys must match.'
		);
	}
	...
	for (let i = 0; i < txns.length; i++) {
		...
		const isValidSignature = algosdk.verifyBytes(
			txnBytes,
			Buffer.from(sig ?? new Uint8Array()),
			publicKey
		);
...
export function getSelectedWalletToken(): string | null {
	...
	const wallet = get(connectedWallets).find(
...
export function signAndSendTransactions(
