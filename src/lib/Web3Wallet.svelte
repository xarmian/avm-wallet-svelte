<script lang="ts">
	...
	import { verifyToken, Wallets } from './wallets.js';
	...
	const closeWalletList = (event: Event) => {
		if (!(event.target as HTMLElement).closest('.wallet-list')) {
	...
	{#if $showWalletList}
		{#if modalType === 'dropdown'}
			<div class="walletListBox absolute right-0 w-72 z-50 {walletListClass}">
				<WalletList {showAuthButtons} {availableWallets} {allowWatchAccounts} />
			</div>
		{:else}
			<div class="fixed inset-0 flex items-center justify-center z-50">
				...
				></div>
				<div
					class="bg-white dark:bg-gray-800 rounded-lg shadow-lg overflow-hidden w-full max-w-lg mx-auto z-10"
				>
					<div
					...
					<div class="p-4">
						<WalletList {showAuthButtons} {availableWallets} {modalType} />
					</div>
				</div>
			</div>
		{/if}
	{/if}
	<SimpleWalletConnectModal
		bind:show={$showModal}
		uri={$connectionUri}
		walletName={$modalWalletName}
	/>
</div>
