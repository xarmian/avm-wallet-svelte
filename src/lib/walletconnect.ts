...
// @ts-expect-error WalletConnect types
	...
	initializationPromise = (async () => {
		try {
			...
			if (lastSession) {
				...
				const storedChainId =
					typeof window !== 'undefined' ? localStorage.getItem('wc-chainId') : null;
				...
				if (!hasRequiredChain && sessionChains.length > 0) {
					if (storedChainId && sessionChains.includes(storedChainId)) {
						...
						if (sessionChainId) {
							console.log(
								`Chain ID mismatch - computed: ${CHAIN_ID}, session: ${sessionChainId}. Using session chain ID.`
							);
				...
				if (
					...
					if (sessionOwner) {
						...
						const wallets = algorandNamespace.accounts.map((account: string) => {
							...
							return { address, app: sessionOwner || 'WalletConnect' };
						});
...
export async function connect(walletName?: string): Promise<WalletConnectionResult[] | null> {
	...
	try {
		...
		// @ts-expect-error WalletConnect ConnectParams type mismatch
