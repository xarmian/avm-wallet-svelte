    ...
    initializationPromise = (async () => {
        try {
            ...
            if (lastSession) {
                ...
                if (algorandNamespace && algorandNamespace.accounts && hasRequiredMethods && hasRequiredChain) {
                    session = lastSession;
...
export async function connect(walletName?: string): Promise<WalletConnectionResult[] | null> {
    ...
    try {
        ...
        if (sessionOwner && sessionOwner !== intendedWalletName && await hasActiveSession()) {
            console.log(`Existing session owned by ${sessionOwner}, disconnecting to connect ${intendedWalletName}...`);
            try {
                await universalProvider.disconnect();
                session = null;
                sessionOwner = null;
...
async function subscribeToEvents(universalProvider: InstanceType<typeof UniversalProvider>) {
    if (!universalProvider)
      throw Error("Unable to subscribe to events. Provider does not exist.");
...
export async function debugWalletConnectState(): Promise<void> {
    ...
    if (provider) {
        ...
        console.log('Provider session namespaces:', JSON.stringify(provider.session?.namespaces, null, 2));
    }
...
export async function signTransactions(txnGroups: algosdk.Transaction[][]) {
    ...
    if (!algorandNamespace.methods?.includes('algo_signTxn')) {
        console.error('❌ algo_signTxn method not found in namespace methods:', algorandNamespace.methods);
        throw new Error("algo_signTxn method not configured. Please reconnect.");
    }

    if (!algorandNamespace.chains?.includes(CHAIN_ID!)) {
        console.error('❌ Chain ID not found in namespace chains:', algorandNamespace.chains, 'Expected:', CHAIN_ID);
    ...
    try {
        ...
        console.log('✅ Signing successful, result type:', typeof result, 'length:', Array.isArray(result) ? result.length : 'not array');
