...
function createSelectedWalletStore(): SelectedWalletStore {
	...
	return {
		...
		set: (wallet: WalletConnectionResult) => {
			const w = get(connectedWallets).find(
				(w) => w.address === wallet.address && w.app === wallet.app
			);
			if (w?.token) {
...
function createWalletStore(): AVMWalletStore {
	...
	return {
		...
		set: (newWallets: WalletConnectionResult[]) => {
			...
			localStorage.setItem(key, JSON.stringify(newWallets));
		},
		remove: (app: string, walletAddress?: string) => {
			...
			const newWallets = storedValue.filter(
				(wallet) =>
					wallet.app !== app || (wallet.address !== walletAddress && wallet.app === 'Watch')
			);
			set(newWallets);

			if (
				get(selectedWallet)?.app === app &&
				(get(selectedWallet)?.address === walletAddress || walletAddress === undefined)
			) {
		...
		add: (wallets: WalletConnectionResult[]) => {
			...
			const newWallets = storedValue
				.concat(wallets)
				.filter(
					(wallet, index, self) =>
						self.findIndex((w) => w.address === wallet.address && w.app === wallet.app) === index
				);
			set(newWallets);
		...
		update: (updater: (wallets: WalletConnectionResult[]) => WalletConnectionResult[]) => {
			...
			newWallets.forEach((wallet) => {
				if (wallet.token) {
					Cookies.set('avm-wallet-token-' + wallet.address, wallet.token, {
						secure: true,
						sameSite: 'strict'
					});
					delete wallet.token;
			...
			if (selected) {
				const newSelected = newWallets.find(
					(w) => w.address === selected.address && w.app === selected.app
				);
				if (newSelected) {
...
}>({ algodClient: undefined, indexerClient: undefined });
export const wcProjectStore = writable<{
	projectId: string;
	projectName: string;
	projectDescription: string;
	projectUrl: string;
	projectIcons: string[];
}>({ projectId: '', projectName: '', projectDescription: '', projectUrl: '', projectIcons: [] });
