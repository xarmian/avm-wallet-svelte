<script lang="ts">
	...
	const connectWallet = async (event: MouseEvent) => {
		...
		try {
			...
			toast.push(e instanceof Error ? e.message : 'Connection failed');
		} finally {
			isLoading = false;
		}
	};
	...
	const disconnectWallet = async (walletAddress?: string) => {
		...
		if (wallet && wallet.disconnect) {
			...
			try {
				...
				toast.push(e instanceof Error ? e.message : 'Disconnection failed');
			} finally {
				isLoading = false;
			}
		} else {
...
{#if wallet}
	<div class="flex flex-col">
		...
		{#if modalType == 'dropdown' && connectedWallets.length > 0}
			<div class="ml-8 mt-1">
				{#each connectedWallets as connectedWallet (connectedWallet.address)}
