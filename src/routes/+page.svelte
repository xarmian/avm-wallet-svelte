<script lang="ts">
	import { Web3Wallet, walletStore, uiStore, uiState, registry } from '$lib/index.js';
		...
		<main class="demo-area">
			<div class="demo-card">
				...
				<p class="demo-description">
					The Web3Wallet component acts as an account selector. Click to see connected accounts
					or add new ones.
				</p>
			...
			<div class="demo-card">
				...
				<p class="demo-description">
					Using a custom trigger button via the snippet prop.
				</p>
				...
				<div class="wallet-container">
					{#key `custom-${displayMode}`}
						<Web3Wallet
							...
							{wcConfig}
						>
							{#snippet trigger({ selectedAccount: acct, toggle, isOpen })}
			...
			<div class="demo-card demo-card--wide">
				...
				<p class="demo-description">
					Create a transaction, sign it with the connected wallet, and verify the signature.
					The transaction will NOT be broadcast to the network.
				</p>
						...
						<div class="tx-result">
							...
							<div class="tx-result-grid">
								...
								<div class="tx-result-item">
									...
									<code class="tx-result-value tx-result-value--mono tx-result-value--truncate">{txResult.sender}</code>
								</div>

								<div class="tx-result-item">
									<span class="tx-result-label">Receiver</span>
									<code class="tx-result-value tx-result-value--mono tx-result-value--truncate">{txResult.receiver}</code>
								</div>
								...
								<div class="tx-result-item">
									...
									<span class="tx-result-value">{txResult.firstValid.toString()} - {txResult.lastValid.toString()}</span>
								</div>
								...
								<div class="tx-result-item">
									...
									<code class="tx-result-value tx-result-value--mono tx-result-value--truncate">{txResult.signature}</code>
								</div>

								<div class="tx-result-item">
									<span class="tx-result-label">Signature Valid</span>
									<span class="tx-result-value tx-result-value--status" class:tx-result-value--valid={txResult.isValid} class:tx-result-value--invalid={!txResult.isValid}>
							...
							<div class="tx-warning">
								<strong>Note:</strong> This transaction was NOT broadcast to the network. It exists only in memory for testing purposes.
							</div>
						</div>
					{/if}
				{/if}
...
<style>
	...
	.custom-trigger {
		...
		background: linear-gradient(135deg, rgb(var(--avm-color-primary)), rgb(var(--avm-color-primary-hover)));
		color: white;
		font-size: 0.875rem;
		font-weight: 500;
		border: none;
